...
def build_import_backed_dashboard(db: Session, dataset_kind: DatasetKind) -> dict[str, object]:
    batches = db.scalars(
        select(ImportBatch)
    ...
    total_rows = sum(batch.accepted_rows for batch in batches)
...
def build_energy_business_dashboard(db: Session) -> dict[str, object]:
    ...
    for batch in technical_batches:
        ...
        supply_values = [
            ...
            if (value := _consumption(cells)) is not None
        ]
        ...
        controlled_kwh = sum(
            value
            for _, cells in supply_rows
            if _is_controlled_supply(_label(cells))
            and (value := _consumption(cells)) is not None
        )
        ...
        external_reported = next(
            (
                ...
                if "сторон" in _normalized_label(cells)
                and _reported_consumption(cells) is not None
            ),
            None,
        )
        ...
        monthly_series.append(
            {
                ...
                "recalculation_difference_kwh": (
                    total_kwh - reported_total_kwh
                    if reported_total_kwh is not None
                    else None
                ),
                "own_kwh": own_kwh,
        ...
        technical_details[period] = {
            ...
            "external_groups": [
                ...
                for name, value in external_groups.items()
            ],
        ...
        for sheet_name, rows in _rows_by_sheet(db, batch.id).items():
            ...
            for _, cells in rows:
                ...
                if (
                    ...
                    and _is_controlled_supply(label)
                    and (current_reading is None or next_reading is None)
                ):
    ...
    top_external = sorted(
        latest_details.get("external_rows", []),
        key=lambda item: float(item["value"]),
        reverse=True,
    )[:8]
    outgoing_35kv = sorted(
        latest_details.get("outgoing_35kv", []),
        key=lambda item: float(item["value"]),
        reverse=True,
    )
    external_groups = sorted(
        latest_details.get("external_groups", []),
        key=lambda item: float(item["value"]),
        reverse=True,
    )
    ...
    latest_reported = (
        float(latest["reported_total_kwh"])
        if latest and latest["reported_total_kwh"] is not None
        else None
    )
    latest_recalculation_difference = (
        latest_total - latest_reported
    ...
    external_share_text = (
        f"{latest_external / latest_total * 100 if latest_total else 0:.1f}".replace(".", ",")
    )
    latest_month_name = (
        MONTHS_PREPOSITIONAL.get(int(str(latest["period"])[5:7]), str(latest["label"]).casefold())
        if latest
        else ""
    )
    insight = (
    ...
    return {
        ...
        "data_quality": {
            ...
            "recalculation_difference_pct": (
                latest_recalculation_difference / latest_reported
                if latest_reported and latest_recalculation_difference is not None
                else None
            ),
        },
        "insight": insight,
